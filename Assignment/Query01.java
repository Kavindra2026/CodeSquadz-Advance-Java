...
class CreateFirstForm {
    ...
    CreateFirstForm() {
        ...
        jbtemp.setBounds(580, 1000, 60, 25);
...

    {
...

        {
...

            {
                ...
                String url = "jdbc:mysql://localhost:3306/" + Connect.dsnName + "?useSSL=false&serverTimezone=UTC";
                Class.forName("com.mysql.cj.jdbc.Driver"); // Good practice to ensure driver is loaded
                Connection c = DriverManager.getConnection(url, "root", "root"); // Use your actual password
            ...
            /* */ cff.jf1.remove(cff.jcom2); // refresing jcom2
...

        {
            ...
            // System.out.println(" ~~ "+sel[0]+" "+sel[2]+" "+sel[3]+" "+sel[4]+"
        ...
        if (ae.getSource() == cff.jb6) {
            ...
            if (cff.createQueryStatus == 0) {
...

                {
                    ...
                    if ((cff.jcom2.getSelectedItem().equals("---None---") == false)
                            ...
                            && (cff.jtf1.getText().equals("---None---") == false))///////////
            ...
            cff.strQuery.append(cff.orderbyBuffer);// now append orderby clause to Queryc
        ...
        if (ae.getSource() == cff.jcb1) {
            ...
            JLabel j11 = new JLabel("Order by");

            j11.setBounds(20, 10, 100, 25);
...
class TableResult {
    ...
    TableResult(StringBuffer sb) {
        ...
        try {
            ...
            c = DriverManager.getConnection(url, "root", "root"); // Use your actual password
...
class Connect {
    ...
    static Vector<String> tableNames = new Vector<>(0, 1);
    static Vector<String> columnNames = new Vector<>(0, 1);
    static Vector<String> fieldNames = new Vector<>(0, 1);
    ...
    Connect() {
        ...
        dbCombo.setBounds(150, 20, 200, 25);
        jf.add(dbCombo);

        jb01 = new JButton("OK");
        jb01.setBounds(70, 90, 100, 25);
        jf.add(jb01);

        jb02 = new JButton("Exit");
        jb02.setBounds(200, 90, 100, 25);
        jf.add(jb02);

        jf.setVisible(true);

        // Load database list from MySQL
        loadDatabaseList();
    ...
    public void loadDatabaseList() {
        try {
            ...
            // connect without any specific database
            ...
            while (rs.next()) {
                dbCombo.addItem(rs.getString(1));
            }
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(jf, "Error loading databases:\n" + e.getMessage());
        }
    }

    public void connectDSN(String selectedDB) {
        try {
            dsnName = selectedDB;
            String url = "jdbc:mysql://localhost:3306/" + dsnName + "?useSSL=false&serverTimezone=UTC";
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection c = DriverManager.getConnection(url, "root", "root");
            System.out.println("Connected to: " + dsnName);
...
class ConnectEvent implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == con.jb01) {
            String selectedDB = con.dbCombo.getSelectedItem().toString();
            if (selectedDB != null && !selectedDB.isEmpty()) {
                con.connectDSN(selectedDB);
                con.jf.dispose();
                new CreateFirstForm();
            } else {
                JOptionPane.showMessageDialog(con.jf, "Please select a database first!");
            }
        }
